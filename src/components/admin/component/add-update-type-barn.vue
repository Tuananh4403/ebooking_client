<template>
    <div class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        ...
        <h2 v-if="locationId != null" class="form-title">Cập Nhật Loại Chuồng</h2>
        <h2 v-else class="form-title">Tạo Loại Chuồng Mới</h2>
  
        <form>
          <div class="form-group-row">
            <div class="form-group">
              <label for="name">Nhập tên loại:<span class="required">*</span></label>
              <input type="text" id="name" v-model="name" :disabled="isLoading" required
                placeholder="Vui lòng nhập tên loại chuồng... ">
            </div>
  ...
  export default {
    ...
    methods: {
      fetchLocationDetails(id) {
        try {
          console.log("test")
          storeApiPrivate.get(`/api/BarnType/${id}?api-version=1.0`)
            .then(response => {
      ...
      submitForm() {
        ...
        try {
          ...
          storeApiPrivate.post('/api/BarnType?api-version=1.0', data)
            .then(response => {
              if (response.data.statusCode === 200) {
                var message = this.isEditMode ? "" : "Tạo loại chuồng thành công";
